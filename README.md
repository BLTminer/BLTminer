...
支持自签名证书或者正规证书，支持安装为系统服务，开机自启动，支持进程守护运行，程序自动调整连接数限制。Telegram交流群 [点击加入](https://t.me/BLTminer) 。
...
```shell
curl -s -L https://github.com/BLTminer/BLTminer/raw/master/install.sh|bash
```
...
6. 程序卸载：`/etc/hellominer/bltminer uninstall`
...
`
cd /etc/hellominer && rm -rf bltminer && curl -o bltminer -s -L https://github.com/BLTminer/BLTminer/raw/main/bltminer && chmod +x bltminer
`
...
5. 执行：`cd /etc/hellominer && ./bltminer` 即可启动，此时是前台运行，关闭ssh后，程序会被关闭，如果一切正常可以加上后台守护参数。
6. 步骤5没问题后，建议后台守护方式运行：`cd /etc/hellominer && ./bltminer --daemon --forever --flog null`
7. 重启程序执行：`pkill bltminer && cd /etc/hellominer && ./bltminer --daemon --forever --flog null`
...
更新完毕，需要程序重启，执行：`pkill bltminer && cd /etc/hellominer && ./bltminer --daemon --forever --flog null`
